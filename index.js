...
client.once('ready', async () => {
    ...
    console.log('✅ Bot Reduto Online e Operacional!');
});

client.on('interactionCreate', async interaction => {
    // SEÇÃO 1: COMANDOS SLASH (CRIAR FILAS)
    ...
    // SEÇÃO 2: BOTÕES DE FILA (ISOLADOS)
    if (interaction.isButton()) {
        ...
        if (['infinito', 'normal', 'sair'].includes(acao)) {
            ...
            return; // Interrompe aqui para não passar pelo sistema antigo
        }
    }
    
    // SEÇÃO 3: SISTEMA ANTIGO (TICKETS E OUTROS)
    // Se o clique não foi de fila, o bot ignora e deixa seu código original (que você deve ter por aí) rodar.
});
